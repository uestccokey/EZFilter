...
class AudioChannel {
    ...
    public AudioChannel(final MediaCodec decoder, final MediaCodec encoder, final MediaFormat encodeFormat) {
        mDecoder = decoder;
        mEncoder = encoder;
    ...
    public void setActualDecodedFormat(final MediaFormat decodedFormat) {
        ...
        int sampleRate = mEncodeFormat.getInteger(MediaFormat.KEY_SAMPLE_RATE);
        if (mInputSampleRate != sampleRate) {
